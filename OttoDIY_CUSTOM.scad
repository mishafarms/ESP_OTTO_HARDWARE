...
brain_type="esp32_shield"; // [nano_shield:Stock (Arduino Nano + Servo Shield),nano_proto:Arduino Nano + Proto board,uno_shield:Arduino Uno + Servo Shield,uno:Arduino Uno (and wires)]
...
body_width=71; // [67:200]

// Body Depth (Head depth is based off this)
body_depth=72; // [68:200]
...
esp32_holes=[[27,7],[-25.2,-26.1],[-23.825,22.25]];
...
function brain_type_text() = (
  ...
  brain_type=="nano_proto"?"Arduino Nano + Proto board":(
  brain_type=="esp32_shield"?"ESP32 Shield":"Unknown"))));
module warnings() {
  ...
  } else if(brain_type=="esp32_shield")
  {
    if( body_width<71)
      error(str("If ",brain_type_text()," is used, Body Width must be >= 71!"));
    else if(body_depth<72)
      error(str("If ",brain_type_text()," is used, Body Depth must be >= 72!"));
    else children();
...
module head(dims=[
  ...
  botm=dims[1]/-2; // -32.25
  right=dims[0]/2; // 31.5
  translate(move?[0,0,body_height+head_height-3]:[0,0,0]) rotate(move?[180,0,-90]:[0,0,0]) {
...
module brain_holes(brain_type=brain_type,
    ...
    back=(body_depth-(body_thickness+part_clearance)*2)/2,
    top=(body_depth-(body_thickness+part_clearance)*2)/2-20) {
    blo=(brain_type=="uno_shield"||brain_type=="uno"?14:(brain_type=="esp32_shield"?3.5:0));
    bto=(brain_type=="uno_shield"?4:(brain_type=="nano_shield"?-.5:(brain_type=="esp32_shield"?-5:0)));
  ...
  {
    translate([right-31.5-blo,top-12.5-bto,brain_screw_thru?-.01:head_thickness]) for(pos=esp32_holes) translate(pos) cylinder(d=brain_screw_size,h=3+head_thickness+.02,$fn=24);
  }
  if(brain_type=="nano_shield") {
    translate([0,top-12])
    {
    ...
    translate([0,back])
    {
        ...
        translate([right-31.5-blo+pwr_offset,-2,hoffset]) pwr_hole();
        translate([right-31.5-blo+usb_offset,-2,hoffset]) usb_micro();
    }
  } else if(brain_type=="uno_shield"||brain_type=="uno") {
...
/*    translate(esp32_holes[0]) translate([-1,2,2]) cube([2,3+top,3]);
      translate([-27-brain_screw_size/2,-8,2]) mirror([1,0])      cube([3-brain_screw_size/2+(right-28),2,3]);
      translate(esp32_holes[1]) translate([-6,-1,2]) cube([4,2,3]);
      translate([22.877,-24,2]) mirror([0,1]) {
        translate([0,1.125]) cube([2,4.875+(top/2+bto-4.6),3.9]);
        cube([2,2,3]);
        translate([1,-1.75,3]) rotate([0,0,180]) cube([7.6,20,3.2], center=true);
      }*/
      translate([0,0,1.99]) for(pos=esp32_holes)
      translate(pos) brain_screw();
